...
```bash
curl -X GET -H "X-API-Key: YOUR_API_KEY" -H "Content-Type: application/json" https://nocobuilder.cloud/api/users/me/usage-limits
```
...
- Überprüfen Sie Ihre aktuelle Nutzung unter [Einstellungen → Abonnement](https://nocobuilder.cloud/settings/subscription)
