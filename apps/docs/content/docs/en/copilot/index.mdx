...
<Callout type="info">
  ...
  1. Go to [nocobuilder.cloud](https://nocobuilder.cloud) → Settings → Copilot and generate a Copilot API key
...
1. Go to [nocobuilder.cloud](https://nocobuilder.cloud) and sign in
...
```json
{
  "mcpServers": {
    "sim-copilot": {
      "url": "https://www.nocobuilder.cloud/api/mcp/copilot",
      "headers": {
...
claude mcp add sim-copilot \
  --transport http \
  https://www.nocobuilder.cloud/api/mcp/copilot \
...
<Callout type="info">
  For self-hosted deployments, replace `https://www.nocobuilder.cloud` with your self-hosted Sim URL.
</Callout>
...
<FAQ items={[
  ...
  { question: "How do I use Copilot from an external editor like Cursor or VS Code?", answer: "You can use Copilot as an MCP server from external editors. First, generate a Copilot API key from Settings > Copilot on nocobuilder.cloud. Then add the MCP server configuration to your editor using the endpoint https://www.nocobuilder.cloud/api/mcp/copilot with your API key in the X-API-Key header. Configuration examples are available for Cursor, Claude Code, Claude Desktop, and VS Code." },
  ...
  { question: "How do I set up Copilot for a self-hosted deployment?", answer: "For self-hosted deployments, go to nocobuilder.cloud > Settings > Copilot and generate a Copilot API key. Then set the COPILOT_API_KEY environment variable in your self-hosted environment. Copilot is a Sim-managed service, so the self-hosted instance communicates with Sim's servers to process requests." },
]} />
