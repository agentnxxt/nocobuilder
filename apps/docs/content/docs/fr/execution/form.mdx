...
- Partagé via un lien direct (par exemple, `https://nocobuilder.cloud/form/my-survey`)
...
3. Configurez :
   - **URL** : identifiant unique (par exemple, `contact-form` → `nocobuilder.cloud/form/contact-form`)
...
```
https://nocobuilder.cloud/form/your-identifier
```
...
```html
<iframe
  src="https://nocobuilder.cloud/form/your-identifier"
  width="100%"
  height="600"
  frameborder="0"
  title="Form"
...
```bash
curl -X POST https://nocobuilder.cloud/api/form/your-identifier \
...
```typescript
const response = await fetch('https://nocobuilder.cloud/api/form/your-identifier', {
  method: 'POST',
...
**Le formulaire ne se charge pas dans l'iframe** - Vérifiez que la CSP de votre site autorise les iframes provenant de `nocobuilder.cloud`.
