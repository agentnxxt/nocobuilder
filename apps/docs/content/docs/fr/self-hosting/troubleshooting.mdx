...
docker compose ps nocobuilder
docker compose logs nocobuilder
...
# Run manually
docker compose exec nocobuilder bun run db:migrate
```
...
docker compose exec nocobuilder apt-get update && apt-get install -y ca-certificates
...
# Specific service
docker compose logs -f nocobuilder
```

## Obtenir de l'aide

- [Problèmes GitHub](https://github.com/nocobuilder/sim/issues)
