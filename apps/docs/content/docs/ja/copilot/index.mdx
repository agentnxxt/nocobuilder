...
<Callout type="info">
  CopilotはSimが管理するサービスです。セルフホスト型のデプロイメントでは、ホスト型アプリ（nocobuilder.cloud → 設定 → Copilot）でCopilot APIキーを生成してください
  1. [nocobuilder.cloud](https://nocobuilder.cloud) → 設定 → Copilotに移動し、Copilot APIキーを生成
...
1. [nocobuilder.cloud](https://nocobuilder.cloud)にアクセスしてサインイン
2. **設定** → **Copilot**に移動
3. **APIキーを生成**をクリック
4. キーをコピー(一度のみ表示されます)
...
<Callout type="info">
  セルフホスト型デプロイメントの場合は、`https://www.nocobuilder.cloud`をあなたのセルフホスト型SimのURLに置き換えてください。
</Callout>
