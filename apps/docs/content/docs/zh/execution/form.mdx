...
- 通过直接链接分享（如：`https://nocobuilder.cloud/form/my-survey`）
- 通过 iframe 嵌入到任意网站
...
2. 选择 **Form** 标签页
3. 配置：
   - **URL**：唯一标识符（如：`contact-form` → `nocobuilder.cloud/form/contact-form`）
   - **Title**：表单标题
   - **Description**：可选副标题
...
**表单无法在 iframe 中加载** - 请检查你的网站 CSP 是否允许来自 `nocobuilder.cloud` 的 iframe。
