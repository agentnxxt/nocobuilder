...
vi.mock('@/lib/copilot/constants', () => ({
  NOCO_AGENT_API_URL_DEFAULT: 'https://agent.sim.example.com',
  NOCO_AGENT_API_URL: 'https://agent.sim.example.com',
}))
...
describe('Copilot API Keys API Route', () => {
  ...
  describe('GET', () => {
    ...
    it('should forward userId to NocoBuilder Agent', async () => {
      mockGetSession.mockResolvedValue({ user: { id: 'user-123', email: 'test@example.com' } })
    ...
    it('should return error when NocoBuilder Agent returns non-ok response', async () => {
      mockGetSession.mockResolvedValue({ user: { id: 'user-123', email: 'test@example.com' } })
    ...
    it('should return 500 when NocoBuilder Agent returns invalid response', async () => {
      mockGetSession.mockResolvedValue({ user: { id: 'user-123', email: 'test@example.com' } })
      ...
      const responseData = await response.json()
      expect(responseData).toEqual({ error: 'Invalid response from NocoBuilder Agent' })
    })
    ...
    it('should handle JSON parsing errors from NocoBuilder Agent', async () => {
      mockGetSession.mockResolvedValue({ user: { id: 'user-123', email: 'test@example.com' } })
  ...
  describe('DELETE', () => {
    ...
    it('should handle JSON parsing errors from NocoBuilder Agent on delete', async () => {
      mockGetSession.mockResolvedValue({ user: { id: 'user-123', email: 'test@example.com' } })
