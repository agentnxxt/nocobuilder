...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${NOCO_AGENT_API_URL}/api/validate-key/get-api-keys`, {
      method: 'POST',
      headers: {
    ...
    if (!Array.isArray(apiKeys)) {
      return NextResponse.json({ error: 'Invalid response from NocoBuilder Agent' }, { status: 500 })
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${NOCO_AGENT_API_URL}/api/validate-key/delete`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(env.COPILOT_API_KEY ? { 'x-api-key': env.COPILOT_API_KEY } : {}),
