...
import { NOCO_AGENT_API_URL } from '@/lib/copilot/constants'
...
export async function GET() {
  try {
    ...
    const res = await fetch(
      `${NOCO_AGENT_API_URL}/api/tool-preferences/auto-allowed?userId=${encodeURIComponent(userId)}`,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${NOCO_AGENT_API_URL}/api/tool-preferences/auto-allowed`, {
      method: 'POST',
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const res = await fetch(
      `${NOCO_AGENT_API_URL}/api/tool-preferences/auto-allowed?userId=${encodeURIComponent(userId)}&toolId=${encodeURIComponent(toolId)}`,
