...
export async function GET() {
  try {
    ...
    const response = await fetch('https://status.nocobuilder.cloud/api/v1/summary', {
      method: 'GET',
      headers: {
    ...
    const statusResponse: StatusResponse = {
      status,
      message,
      url: data.page_url || 'https://status.nocobuilder.cloud',
    ...
    const errorResponse: StatusResponse = {
      ...
      url: 'https://status.nocobuilder.cloud',
      lastUpdated: new Date().toISOString(),
    }
