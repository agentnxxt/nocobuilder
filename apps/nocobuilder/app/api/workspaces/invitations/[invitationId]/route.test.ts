...
vi.mock('@/lib/core/utils/urls', () => ({
  getBaseUrl: vi.fn().mockReturnValue('https://test.nocobuilder.cloud'),
}))
  ...
  describe('GET /api/workspaces/invitations/[invitationId]', () => {
    ...
    it('should redirect to login when unauthenticated with token', async () => {
      ...
      expect(response.headers.get('location')).toBe(
        'https://test.nocobuilder.cloud/invite/token-abc123?token=token-abc123'
      )
    })
    ...
    it('should accept invitation when called with valid token', async () => {
      ...
      expect(response.status).toBe(307)
      expect(response.headers.get('location')).toBe(
        'https://test.nocobuilder.cloud/workspace/workspace-456/home'
      )
    })
    ...
    it('should redirect to error page with token preserved when invitation expired', async () => {
      ...
      expect(location).toBe(
        'https://test.nocobuilder.cloud/invite/invitation-789?error=expired&token=token-abc123'
      )
    })
    ...
    it('should redirect to error page with token preserved when email mismatch', async () => {
      ...
      const location = response.headers.get('location')
      expect(location).toBe(
        'https://test.nocobuilder.cloud/invite/invitation-789?error=email-mismatch&token=token-abc123'
      )
    })
    ...
    it('should redirect to error page with token preserved when invitation not found (with token)', async () => {
      ...
      const location = response.headers.get('location')
      expect(location).toBe(
        'https://test.nocobuilder.cloud/invite/non-existent?error=invalid-token&token=some-invalid-token'
      )
    })
    ...
    it('should redirect to error page with token preserved when invitation already processed', async () => {
      ...
      const location = response.headers.get('location')
      expect(location).toBe(
        'https://test.nocobuilder.cloud/invite/invitation-789?error=already-processed&token=token-abc123'
      )
    })
    ...
    it('should redirect to error page with token preserved when workspace not found', async () => {
      ...
      const location = response.headers.get('location')
      expect(location).toBe(
        'https://test.nocobuilder.cloud/invite/invitation-789?error=workspace-not-found&token=token-abc123'
      )
    })
    ...
    it('should redirect to error page with token preserved when user not found', async () => {
      ...
      const location = response.headers.get('location')
      expect(location).toBe(
        'https://test.nocobuilder.cloud/invite/invitation-789?error=user-not-found&token=token-abc123'
      )
    })
