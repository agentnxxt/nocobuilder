 ...
 *
 * Usage:
 *   bun run apps/nocobuilder/scripts/check-subblock-id-stability.ts [base-ref]
 *
 * base-ref defaults to HEAD~1.  In a PR CI pipeline, pass the merge base:
 *   bun run apps/nocobuilder/scripts/check-subblock-id-stability.ts origin/main
 */
...
function getPreviousIds(): IdMap {
  const registryPath = 'apps/nocobuilder/blocks/registry.ts'
  const blocksDir = 'apps/nocobuilder/blocks/blocks'
