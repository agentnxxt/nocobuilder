...
async function finalizeStream(
  ...
  if (initiated.kind === 'success') {
    ...
    if (result.status === 401) {
      errorContent =
        '_Unauthorized request. You need a valid API key to use the copilot. You can get one by going to [nocobuilder.cloud](https://nocobuilder.cloud) settings and generating one there._'
