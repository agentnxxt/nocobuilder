...
helm install sim ./helm/nocobuilder
```
...
```bash
helm install sim ./helm/nocobuilder -f custom-values.yaml
```
...
```bash
helm install sim-dev ./helm/nocobuilder \
  --values ./helm/nocobuilder/examples/values-development.yaml \
  --namespace nocobuilder-dev --create-namespace
```

### Production Environment

```bash
helm install sim-prod ./helm/nocobuilder \
  --values ./helm/nocobuilder/examples/values-production.yaml \
  --namespace nocobuilder-prod --create-namespace
```

### Azure Environment

```bash
helm install sim-azure ./helm/nocobuilder \
  --values ./helm/nocobuilder/examples/values-azure.yaml \
  --namespace nocobuilder --create-namespace
```

### AWS Environment (EKS)

```bash
helm install sim-aws ./helm/nocobuilder \
  --values ./helm/nocobuilder/examples/values-aws.yaml \
  --namespace nocobuilder --create-namespace
```
...
```bash
helm install sim-gcp ./helm/nocobuilder \
  --values ./helm/nocobuilder/examples/values-gcp.yaml \
  --namespace nocobuilder --create-namespace
```
...
```bash
helm install sim-prod ./helm/nocobuilder \
  --values ./helm/nocobuilder/examples/values-external-db.yaml \
  --set externalDatabase.host="your-rds-endpoint.com" \
  --set externalDatabase.username="nocobuilder_user" \
  --set externalDatabase.password="secure-password" \
  --set externalDatabase.database="nocobuilder_prod" \
  --namespace nocobuilder --create-namespace
```
...
| `global.useRegistryForAllImages` | Use custom registry for all images (not just nocobuilder/*) | `false` |
...
| `app.image.repository` | App image repository | `nocobuilder/sim` |
...
| `realtime.image.repository` | Realtime image repository | `nocobuilder/realtime` |
...
| `migrations.image.repository` | Migrations image repository | `nocobuilder/migrations` |
...
```bash
helm upgrade sim ./helm/nocobuilder
```
...
ingress:
  ...
  tls:
    enabled: true
    secretName: "nocobuilder-tls-secret"
```
...
- GitHub Issues: https://github.com/nocobuilder/sim/issues
